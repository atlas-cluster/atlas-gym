...
export async function deleteRoom(
  ...
  try {
    ...
    const result = await pool.query(
      ...
      log_room AS (
        INSERT INTO audit_logs (room_id, action, entity_id, entity_type, description)
        SELECT $2, 'Delete'::action_type, dm.id, 'room',
          'Room ' || tm.firstname || ' ' || tm.lastname || ' deleted'
        FROM deleted_room dm, target_room tm
      )
      SELECT
