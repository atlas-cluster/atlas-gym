  ...
  try {
    ...
    if (!member.isTrainer) {
      return {
        success: false,
        errorType: 'AUTH',
        message: 'Only trainers can uncancel sessions.',
      }
    }
      ...
      `WITH target_session AS (
       ...
       updated_session AS (
         UPDATE course_sessions
           SET is_cancelled = false, updated_at = NOW()
           WHERE id = $1
       ...
       log_session AS (
         ...
         SELECT $2, 'Update'::action_type, us.id, 'course_session',
           'Session ' || ts.session_name || ' uncancelled'
    ...
    console.error('[UNCANCEL_SESSION_ERROR]:', error)
    return {
      success: false,
      errorType: 'UNKNOWN',
