...
export function SubscriptionGrid({
  ...
  const sortedPlans = [...allPlansToDisplay].sort((a, b) => {
    ...
    const aIsActive = a.subscription && (a.subscription.status === 'active' || a.subscription.status === 'cancelled')
    const bIsActive = b.subscription && (b.subscription.status === 'active' || b.subscription.status === 'cancelled')
