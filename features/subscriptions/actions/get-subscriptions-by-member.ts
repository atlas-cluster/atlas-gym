...
export async function getSubscriptionsByMember(
  ...
  // Only trainers can view other members' subscriptions
  if (!session.member.isTrainer && session.member.id !== memberId) {
    throw new Error('Unauthorized: Only trainers can view other members')
  }
